...
| `dr_ingest` | Shared ingestion utilities that classify/parses WandB runs, apply normalization via declarative configs, and expose QA extraction/transform helpers. | `src/dr_ingest/wandb/classifier.py`, `src/dr_ingest/wandb/postprocess.py`, `src/dr_ingest/qa/*`, `configs/wandb/*.cfg`. | Cleaned run metadata DataFrames, QA instance payloads; planned DuckDB tables. |
