...
| `dr_showntell` | WandB parsing + enrichment playground. Regex-based run classification, merging with pretraining metrics, producing matched finetune/post-train tables. | `src/dr_showntell/run_id_parsing.py`, `notebooks/combine_processed_with_pretrain_data.py`. | `data/*_modernized_run_data.pkl`, `data/combined_plotting_data_*.pkl`. |
